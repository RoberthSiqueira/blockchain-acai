...
function purchaseOrder(purchaseOrder) {
    ...
    console.log('Adding purchase order ' + purchaseOrder.order + ' to shipment ' + shipment.$identifier);
    console.log(shipment);

    if (purchaseOrder.order > 0 
        && purchaseOrder.order < shipment.unitCount 
        && shipment.status == 'ARRIVED') {
        shipment.unitCount -= purchaseOrder.order;
        var price = (purchaseOrder.order * contract.unitPrice);
